...
def generate_pdf(date_str: str, output_path: Optional[str] = None) -> str:
    ...
    class _PosturePDF(FPDF):
        def footer(self):  # called automatically on each page
            ...
            self.cell(
                0, 5,
                _pdf_safe(f"{APP_NAME}  |  {_label}  |  Generated {_gen_time}  |  Page {self.page_no()}"),
                align="C",
            )
...
def generate_monthly_pdf(year: int, month: int, output_path: Optional[str] = None) -> str:
    ...
    pdf.set_text_color(255, 255, 255)
    pdf.set_xy(10, 8)
    pdf.cell(0, 10, _pdf_safe(f"{APP_NAME} - Monthly Report"), ln=True)
...
def send_email(
    ...
    msg = EmailMessage()
    msg["Subject"] = f"{APP_NAME} — Daily Report ({date_str})"
    msg["From"]    = smtp_user
    msg["To"]      = to_addr
    msg.set_content(
        f"Hi,\n\n"
...
def share_whatsapp_web(pdf_path: str, phone: str = "") -> None:
    ...
    msg = (
        ...
        f"Generated by {APP_NAME} on {datetime.now().strftime('%d %b %Y')}."
    )
